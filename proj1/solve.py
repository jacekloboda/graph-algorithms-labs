...
"""
fast factorization found on stack overflow
"""
...
def gen_res_graph(scores):
    ...
    graph structure
    (0, 0) - sink, (0, 1) = hole
    (d, 0) - divider from scores
    (f, n) - prime factor f to the power of n
    """
    ...
    for divider, luck in scores:
        ...
        for factor, cnt in factorization(divider).items():
            prev = div_node
            while cnt > 0:
                factor_node = (factor, cnt)
                M[prev][factor_node] = float("inf")
                M[factor_node][t] = 5 * log(factor, 10)
                cnt -= 1
                prev = factor_node
...
"""
variation of Edmonds Karp algorithm
"""
